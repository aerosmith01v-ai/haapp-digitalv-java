 ...
 *   Node.js v6.0  ←→  Python v1.x  ←→  Java v1.0 (esta)
 * + Keep-alive para mantener vivo el servicio en Render
 */
@Service
public class SincronizacionService {
    ...
    // ✅ Jackson para serialización segura
    ...
    // ============================================================
    // KEEP-ALIVE — cada 14 minutos para no dormirse en Render
    // ============================================================
    ...
    public void keepAlive() {
        ...
        try {
            ...
            log.info("💓 [keep-alive] HaaPpDigitalV Java → vivo ✅ ({})", resp.statusCode());
        } catch (Exception e) {
            log.warn("⚠️  [keep-alive] Sin respuesta propia: {}", e.getMessage());
        }
    }

    // ============================================================
    // SINCRONIZACIÓN — cada 5 minutos con hermanas SOFI
    // ============================================================
    ...
    private void enviarPatrones(String nombre, String url) {
        try {
            ...
            // ✅ Jackson — serialización segura, sin bugs con caracteres especiales
            ...
            HttpResponse<String> resp = httpClient.send(req, HttpResponse.BodyHandlers.ofString());
            log.info("📤 {} patrones → SOFI {} (status {})", ultimos.size(), nombre, resp.statusCode());
        } catch (Exception e) {
