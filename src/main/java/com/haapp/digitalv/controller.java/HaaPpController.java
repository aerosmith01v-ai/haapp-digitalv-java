...
public class HaaPpController {
    ...
    public Map<String, Object> health() {
        return Map.of(
            "status","OK","app","HaaPpDigitalV v1.0 — Java Spring Boot",
            "hz",hzKuhul,"voz",voz.estado(),"red_sofi",sync.getEstadoRed(),
            "modulos",List.of("mapa","vaiven","frecuencias","suenos","tiktok","vendedor","voz_local","sync"),
            "frase","Tecnología con alma maya","timestamp",System.currentTimeMillis()
        );
    }
    ...
    public Map<String, Object> zonaCercana(@RequestParam double lat, @RequestParam double lng) {
        Map<String, Object> cercana = ZONAS_KUHUL.stream()
            .min(Comparator.comparingDouble(z -> distancia(lat,lng,(double)z.get("lat"),(double)z.get("lng"))))
        ...
        for (Map<String, Object> ruta : RUTAS_VAIVEN) {
            ...
            paraderos.stream()
                ...
                .ifPresent(p -> {
                    ...
                    r.put("ruta", ruta.get("nombre")); r.put("id", ruta.get("id"));
                    r.put("color", ruta.get("color")); r.put("paradero", p.get("nombre"));
                    r.put("distancia_km", String.format("%.2f",distancia(lat,lng,(double)p.get("lat"),(double)p.get("lng"))));
                    resultado.add(r);
                });
        }
    ...
    public Map<String, Object> analizarFrecuencia(@RequestBody Map<String, Object> body) {
        ...
        if (diff < 0.5)      { estado="RESONANCIA K'UHUL";  recomendacion="Estás en sincronía con la frecuencia maya."; }
        else if (diff < 2.0) { estado="AFINANDO";           recomendacion="Cerca de la resonancia. Respira profundo."; }
        else if (hz < 8)     { estado="DELTA / SUEÑO";      recomendacion="Frecuencia de descanso profundo."; }
        else if (hz < 13)    { estado="ALPHA / CALMA";      recomendacion="Estado de calma consciente."; }
        else                 { estado="BETA / ACTIVO";      recomendacion="Alta actividad mental."; }
        ...
        return Map.of("hz_detectado",hz,"hz_kuhul",hzKuhul,"estado",estado,
            "recomendacion",recomendacion,"coherencia",String.format("%.3f",coherencia));
    }
    ...
    public Map<String, Object> registrarSueno(@RequestBody Map<String, Object> body) {
        ...
        sueno.put("descripcion",descripcion); sueno.put("usuario_id",userId);
        sueno.put("fecha",new Date().toInstant().toString());
        sueno.put("hz",hzKuhul); sueno.put("analisis",analizarSueno(descripcion));
        suenosDB.add(sueno);
    ...
    public Map<String, Object> obtenerSuenos(@PathVariable String userId) {
        List<Map<String, Object>> suenos = suenosDB.stream()
            .filter(s -> userId.equals(s.get("usuario_id"))).toList();
    ...
    public ResponseEntity<?> generarTiktok(@RequestBody Map<String, Object> body) {
        ...
        return ResponseEntity.ok(Map.of("tema",tema,"hook",generarHook(tema),
            "hashtags",generarHashtags(tema,nicho),
            "tip_algoritmo","Publica entre 7-9pm CST para mayor alcance en Yucatán"));
    }
    ...
    public ResponseEntity<?> generarVendedor(@RequestBody Map<String, Object> body) {
        ...
        return ResponseEntity.ok(Map.of("producto",producto,
            ...
            "copy",producto+" — calidad garantizada. Solo $"+precio+" MXN.",
    ...
    public Map<String, Object> estadoCompleto() {
        return Map.of("app","HaaPpDigitalV Java v1.0",
            "hz",hzKuhul+Math.sin(System.currentTimeMillis()/10000.0)*0.05,
            "version","java-1.0","patrones",sync.getPatrones().size(),
            "red",sync.getEstadoRed(),"timestamp",new Date().toInstant().toString());
    }

    @GetMapping("/api/planes")
    public Map<String, Object> planes() {
        return Map.of("planes", List.of(
            Map.of("id","free","nombre","Free","precio",0,"moneda","MXN",
                "funciones",List.of("Mapa K'uhul","Chat","Sueños","Frecuencias","Va y Ven")),
            Map.of("id","pro","nombre","Pro","precio",99,"moneda","MXN",
                "funciones",List.of("Todo Free","TikTok generator","Kit vendedor","Voz local")),
            Map.of("id","kuhul","nombre","K'uhul","precio",299,"moneda","MXN",
                "funciones",List.of("Todo Pro","Análisis frecuencial IA","Agente de ventas"))
        ));
    }
    ...
    private Map<String, Object> analizarSueno(String texto) {
        ...
        if (t.contains("agua")||t.contains("cenote"))     simbolos.add(Map.of("simbolo","Agua","significado","Purificación maya"));
        if (t.contains("serpiente")||t.contains("kukulcan")) simbolos.add(Map.of("simbolo","Kukulcán","significado","Transformación"));
        if (t.contains("jaguar"))                          simbolos.add(Map.of("simbolo","Jaguar","significado","Poder Xibalbá"));
        if (t.contains("luz")||t.contains("sol"))         simbolos.add(Map.of("simbolo","Kinich Ahau","significado","Claridad"));
        if (t.contains("pirámide")||t.contains("templo")) simbolos.add(Map.of("simbolo","Pirámide","significado","Conexión cósmica"));
        if (t.contains("volar")||t.contains("vuelo"))     simbolos.add(Map.of("simbolo","Vuelo","significado","Ascenso espiritual"));
        String mensaje = simbolos.isEmpty() ? "Sueño de procesamiento cotidiano."
